...
async def verify_login_and_get_profile(page, task_id, user_id):
    ...
    try:
        ...
        # Check if we're logged in by looking for login form or profile
        login_indicators = await page.query_selector('input[name="email"], input[name="pass"], #loginform')
        profile_indicators = await page.query_selector('[aria-label="Facebook"][role="navigation"], [data-testid="blue_bar_root"]')
        
        if login_indicators:
            log_console(f"[{task_id}] ❌ LOGIN FAILED - Showing login form", user_id)
            return {'success': False, 'name': 'Login Failed', 'id': 'Unknown'}
        
        if not profile_indicators:
            log_console(f"[{task_id}] ⚠️ Cannot determine login status", user_id)
        
        log_console(f"[{task_id}] ✅ Successfully logged in!", user_id)
        
        # Now get profile information
        profile_info = await extract_profile_name(page, task_id, user_id)
        return profile_info
...
async def extract_profile_name(page, task_id, user_id):
