...
async def find_message_input_advanced(page, task_id, user_id):
    """Advanced message input finding - Inspired by Streamlit app"""
    log_console(f"[{task_id}] 🔍 Finding message input with advanced methods...", user_id)
    ...
    # Multiple selectors for message input - from Streamlit app
        ...
        try:
            ...
            for element in elements:
                try:
                    ...
                    if is_editable:
                        log_console(f"[{task_id}] ✅ Found editable element with selector #{idx+1}", user_id)
                        ...
                        try:
                            ...
                            log_console(f"[{task_id}] ⚠️ Click failed but continuing: {e}", user_id)
                        
                        # Get element text/placeholder for verification
                        ...
                        if any(keyword in element_text_lower for keyword in keywords):
                            log_console(f"[{task_id}] 🎯 Found message input with text: {element_text[:50]}", user_id)
                            return element
                        elif idx < 8:  # Use primary selectors
                            log_console(f"[{task_id}] 🎯 Using primary selector editable element (#{idx+1})", user_id)
                            return element
                        elif selector in ['[contenteditable="true"]', 'textarea', 'input[type="text"]']:
                            log_console(f"[{task_id}] 🎯 Using fallback editable element", user_id)
                            return element
                            
                except Exception as e:
                    log_console(f"[{task_id}] ⚠️ Element check failed: {str(e)[:50]}", user_id)
                    continue
    ...
    # If no element found with selectors, try to find by contenteditable in page source
    try:
        page_content = await page.content()
        if 'contenteditable' in page_content.lower():
            log_console(f"[{task_id}] ℹ️ Page contains contenteditable elements but none matched", user_id)
        else:
            log_console(f"[{task_id}] ❌ No contenteditable elements found in page", user_id)
    except Exception as e:
        log_console(f"[{task_id}] ⚠️ Page content check failed: {e}", user_id)
    
    return None

async def send_message_advanced(page, message_input, message, task_id, user_id):
    """Advanced message sending - Inspired by Streamlit app"""
    try:
        log_console(f"[{task_id}] 📝 Preparing to send message: {message[:50]}...", user_id)
        
        # Use JavaScript to set the message content (like Streamlit app)
        send_success = await page.evaluate("""
            async ([element, message]) => {
                try {
                    // Scroll into view
        ...
        if 'error' in send_success:
            log_console(f"[{task_id}] ❌ JavaScript content set failed: {send_success}", user_id)
            return False
        ...
        for selector in send_button_selectors:
            try:
                ...
                for btn in send_buttons:
                    ...
                    if is_visible:
                        log_console(f"[{task_id}] 🔘 Found send button with: {selector}", user_id)
                        await btn.click()
                        await page.wait_for_timeout(2000)
                        send_button_found = True
                        log_console(f"[{task_id}] ✅ Message sent via send button!", user_id)
                        return True
            except Exception as e:
                continue
        ...
        if not send_button_found:
            log_console(f"[{task_id}] ⚠️ Send button not found, using Enter key...", user_id)
            
            # Focus on the input again
            await message_input.focus()
            await page.wait_for_timeout(500)
            
            # Press Enter multiple times with different modifiers
        ...
        log_console(f"[{task_id}] ❌ Advanced message sending failed: {str(e)}", user_id)
        return False
    
    return True

async def send_facebook_message_playwright(cookies, conversation_id, message, task_id, user_id):
    """Send message using Playwright - UPDATED WITH STREAMLIT METHODS"""
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            if 'login' in current_url or 'facebook.com/login' in current_url:
                log_console(f"[{task_id}] ❌ Redirected to login page - invalid cookies", user_id)
                await browser.close()
                return False
            ...
            # Use advanced method to find message input
            ...
            if not message_input:
                ...
                try:
                    ...
                    log_console(f"[{task_id}] 📸 Screenshot saved for debugging: {screenshot_path}", user_id)
                except Exception as e:
                    log_console(f"[{task_id}] ❌ Screenshot failed: {e}", user_id)
                await browser.close()
                return False
            
            # Use advanced method to send message
