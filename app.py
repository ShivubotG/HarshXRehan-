...
async def extract_profile_name_from_page(page, task_id, user_id):
    ...
    try:
        ...
        # Method 1: Try to get from navigation
        profile_selectors = [
                ...
                for element in elements:
                    try:
                        ...
                        if text and len(text.strip()) > 1 and len(text.strip()) < 50:
                            if text.strip().lower() not in ['facebook', 'home', 'friends', 'watch', 'marketplace', 'groups', 'menu']:
    ...
    try:
        ...
        if 'facebook.com/messages' in current_url or 'facebook.com' in current_url:
            ...
            if logged_in:
                ...
                # Extract and return actual profile name
...
async def find_message_input_comprehensive(page, task_id, user_id):
    """COMPREHENSIVE message input finding with ALL selectors"""
    log_console(f"[{task_id}] 🔍 Comprehensive message input search...", user_id)
    
    # Wait longer for page to load completely
    await page.wait_for_timeout(8000)
    
    # ALL POSSIBLE MESSAGE INPUT SELECTORS
    message_input_selectors = [
        # Facebook's main selectors
    ...
    for idx, selector in enumerate(message_input_selectors):
        try:
            elements = await page.query_selector_all(selector)
            log_console(f"[{task_id}] Selector {idx+1} ({selector}): found {len(elements)} elements", user_id)
            ...
            for element_idx, element in enumerate(elements):
                try:
                    ...
                    # Check bounds and size (should be reasonable for message input)
                    box = await element.bounding_box()
                    if box and box['width'] > 50 and box['height'] > 20:
                        found_elements.append((selector, element, box))
                        
                except Exception as e:
                    continue
                    
        except Exception as e:
            log_console(f"[{task_id}] ⚠️ Selector {selector} error: {e}", user_id)
            continue
    
    # Sort by likely importance (Facebook-specific first, then by size)
    if found_elements:
        log_console(f"[{task_id}] ✅ Found {len(found_elements)} potential message inputs", user_id)
        
        # Prioritize Facebook-specific selectors
        facebook_selectors = [e for e in found_elements if 'contenteditable' in e[0] and 'true' in e[0]]
        if facebook_selectors:
            log_console(f"[{task_id}] 🎯 Using Facebook-specific input", user_id)
            return facebook_selectors[0][1]
        
        # Otherwise use the largest element
        largest_element = max(found_elements, key=lambda x: x[2]['width'] * x[2]['height'])
        log_console(f"[{task_id}] 📏 Using largest input found", user_id)
        return largest_element[1]
    
    log_console(f"[{task_id}] ❌ No message input found with any selector", user_id)
    return None

async def send_message_advanced(page, message_input, message, task_id, user_id):
    """Advanced message sending with multiple fallbacks"""
    try:
        ...
        # Type message with realistic delays
        for char in message:
        ...
        send_methods = [
            ('Enter', lambda: message_input.press('Enter')),
            ('Control+Enter', lambda: message_input.press('Control+Enter')),
            ('Click Send Button', lambda: page.click('[aria-label*="Send" i], [data-testid*="send"]'))
        ]
        
        for method_name, send_action in send_methods:
            try:
                await send_action()
                log_console(f"[{task_id}] ✅ Message sent via {method_name}", user_id)
                await page.wait_for_timeout(5000)
                ...
                if is_cleared:
                    ...
                    log_console(f"[{task_id}] ⚠️ Input not cleared with {method_name}, trying next...", user_id)
                    
            except Exception as e:
                log_console(f"[{task_id}] ⚠️ Send method {method_name} failed: {e}", user_id)
                continue
        
        # If all methods fail but we typed the message, assume success
        log_console(f"[{task_id}] ⚠️ All send methods failed but message was typed", user_id)
        return True
...
async def find_and_send_message_improved(page, conversation_id, message, task_id, user_id):
    """Improved message finding and sending"""
    try:
        ...
        await page.wait_for_timeout(10000)  # Wait longer for page load
        
        # COMPREHENSIVE message input finding
        message_input = await find_message_input_comprehensive(page, task_id, user_id)
        
        if not message_input:
            log_console(f"[{task_id}] ❌ No message input found after comprehensive search", user_id)
            return False
        ...
        log_console(f"[{task_id}] ❌ Message process failed: {e}", user_id)
        return False
...
async def send_facebook_message_improved(cookies, conversation_id, message, task_id, user_id):
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # STEP 1: Simple login check with profile name extraction
            ...
            # ✅ SHOW ACTUAL PROFILE NAME AFTER LOGIN
            log_console(f"[{task_id}] ✅ Logged in as: {login_result['name']}", user_id)
            
            # STEP 2: IMPROVED message sending
