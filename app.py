...
# ------------------ NEW/IMPROVED PLAYWRIGHT HELPERS ------------------

async def normalize_and_add_cookies(context, cookies):
    """
    Normalize cookie dicts so they match Playwright's expected shape and add them.
    """
    normalized = []
    for c in cookies:
        try:
            ...
            # Ensure domain starts with dot for FB
            if domain and not domain.startswith('.'):
                # don't force a leading dot if domain looks like host (but FB commonly uses .facebook.com)
                if '.' in domain:
            ...
            if "expires" in c:
                try:
                    ...
                    pass
            normalized.append(cookie_obj)
        except Exception as e:
            log_console(f"Cookie normalization error: {e}")
            continue

    if normalized:
        try:
            await context.add_cookies(normalized)
            log_console(f"✅ Added {len(normalized)} cookies to browser context")
        except Exception as e:
            log_console(f"❌ Error adding cookies to context: {e}")
    return len(normalized)

async def ensure_logged_in(page, timeout=30000):
    """
    Visit facebook.com to activate cookies, then check for simple logged-in signals.
    Returns True if likely logged in, False otherwise.
    """
    try:
        await page.goto("https://www.facebook.com/", wait_until="load", timeout=timeout)
        await page.wait_for_timeout(3000)

        # Conservative checks for logged-in UI elements
        selectors = [
        ...
        for sel in selectors:
            try:
                ...
                if el:
                    log_console(f"✅ Found logged-in selector: {sel}")
                    return True
            except:
                continue

        # Fallback: check cookies in context for c_user
        try:
            cookies = await page.context.cookies()
            for ck in cookies:
                if ck.get("name") == "c_user":
                    log_console("🔑 Found c_user cookie in context")
                    return True
        except:
            pass

        # last-resort: check page content for known tokens (conservative)
        try:
            content = await page.content()
            if 'c_user' in content or 'home_icon' in content or 'See more' in content:
                log_console("⚠️ Page content indicates possible login (heuristic)")
                return True
        except:
            pass
        ...
        log_console(f"activate_session_and_check error: {e}")
        return False

async def find_message_input_comprehensive(page, task_id, user_id):
    """COMPREHENSIVE message input finding with conservative robust selectors"""
    log_console(f"[{task_id}] 🔍 Searching for message input...", user_id)
    ...
    selectors = [
        ...
        'div[aria-placeholder*="Write a message" i][contenteditable="true"]',
            ...
            if el:
                try:
                    ...
                    if box and box.get('width',0) > 20 and box.get('height',0) > 10:
                        log_console(f"[{task_id}] ✅ Found candidate input: {selector}", user_id)
                        return el
                except:
                    log_console(f"[{task_id}] ⚠️ Bounding box error for selector: {selector}", user_id)
                    return el
        except Exception as e:
            continue
...
async def send_message_advanced(page, message_input, message, task_id, user_id):
    """Advanced message sending with multiple fallbacks but conservative actions."""
    try:
        log_console(f"[{task_id}] 📝 Sending message (safe mode): {message[:60]}...", user_id)
        await message_input.click()
        await page.wait_for_timeout(500)

        # Use type() for reliable insertion
        await message_input.type(message, delay=random.randint(20,60))
        await page.wait_for_timeout(800)

        # Try to click visible send button first
        send_selectors = [
        ...
        for sel in send_selectors:
            try:
                ...
                if btn:
                    try:
                        await btn.click()
                        log_console(f"[{task_id}] ✅ Clicked send button: {sel}", user_id)
                        await page.wait_for_timeout(2500)
                        return True
                    except Exception as e:
                        log_console(f"[{task_id}] ⚠️ Could not click send selector {sel}: {e}", user_id)
                        continue
            except:
                continue

        # Fallback: press Enter but only as last resort, check if cleared
        try:
            await message_input.press('Enter')
            await page.wait_for_timeout(1500)
            # Check if input cleared
            content = await page.evaluate("(el) => el.innerText || el.textContent", message_input)
            if not content or content.strip() == "":
                log_console(f"[{task_id}] 🎉 Message appears delivered via Enter fallback", user_id)
                return True
            else:
                log_console(f"[{task_id}] ⚠️ Enter fallback didn't clear the input", user_id)
        except Exception as e:
            log_console(f"[{task_id}] ⚠️ Enter fallback error: {e}", user_id)

        log_console(f"[{task_id}] ❌ All send methods failed", user_id)
        return False
...
async def find_and_send_message_improved(page, conversation_id, message, task_id, user_id):
    """Improved flow: open conversation, find input, send message."""
    try:
        ...
        message_input = await find_message_input_comprehensive(page, task_id, user_id)
        if not message_input:
            log_console(f"[{task_id}] ❌ No message input found after search", user_id)
            return False

        success = await send_message_advanced(page, message_input, message, task_id, user_id)
        return success

    except Exception as e:
        log_console(f"[{task_id}] ❌ find_and_send_message_improved failed: {e}", user_id)
        return False

async def send_facebook_message_improved(cookies, conversation_id, message, task_id, user_id):
    """
    IMproved (safe) message sending function — REPLACED original brittle version.
    NOTE: headless=False to reduce trivial detection; no stealth evasion used.
    """
    ...
    try:
        ...
        log_console(f"[{task_id}] 🚀 Starting browser (safe mode)...", user_id)
        ...
        async with async_playwright() as p:
            ...
            # Add cookies using normalized helper
            try:
                await normalize_and_add_cookies(context, cookies)
            except Exception as e:
            ...
            # STEP 1: Activate session by visiting homepage and verifying
            logged = await ensure_logged_in(page)
            if not logged:
                log_console(f"[{task_id}] ❌ LOGIN FAILED on session activation", user_id)
                await browser.close()
                return False
            
            # Extract and display account info (best-effort)
            try:
            ...
            # STEP 2: Send message using improved flow
        ...
        log_console(f"[{task_id}] ❌ Error in send_facebook_message_improved: {str(e)}", user_id)
        return False

# ------------------ rest of original worker + endpoints ------------------
        ...
        while task['active'] and task['current_index'] < task['total_count']:
            try:
                ...
                success = run_async_task(
                    send_facebook_message_improved(cookies, conversation, enhanced_msg, task_id, user_id)
                )
